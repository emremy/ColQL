...
class BooleanChunk {
  ...
  constructor(readonly capacity: number, bytes?: Uint8Array) {
    this.bytes = bytes ?? new Uint8Array(Math.ceil(capacity / BITS_PER_BYTE));
  }
  ...
  deleteAt(offset: number, length: number): void {
    ...
    if (length > 0) this.set(length - 1, false);
  }

  copyInto(target: Uint8Array, targetBitOffset: number, length: number): void {
    for (let index = 0; index < length; index += 1) {
      const bitIndex = targetBitOffset + index;
      const byteIndex = Math.floor(bitIndex / BITS_PER_BYTE);
      const mask = 1 << (bitIndex % BITS_PER_BYTE);
      if (this.get(index)) target[byteIndex] |= mask;
      else target[byteIndex] &= ~mask;
    }
  }
}
...
export class BooleanColumnStorage implements ColumnStorage<boolean> {
  ...
  constructor(
    capacity: number,
    bytes?: Uint8Array,
    rowCount = bytes === undefined ? 0 : capacity,
    ...
    if (bytes !== undefined) {
      ...
      if (bytes.length !== expectedBytes) {
        throw new ColQLError("COLQL_INVALID_SERIALIZED_DATA", `Boolean column byte length ${bytes.length} does not match capacity ${capacity}.`);
      }
      const logicalLength = Math.min(rowCount, capacity);
      for (let index = 0; index < logicalLength; index += 1) {
        const byteIndex = Math.floor(index / BITS_PER_BYTE);
        const bitOffset = index % BITS_PER_BYTE;
        this.append((bytes[byteIndex] & (1 << bitOffset)) !== 0);
      }
    }
  }
  ...
  toBytes(): Uint8Array {
    const output = new Uint8Array(Math.ceil(this.logicalCapacity / BITS_PER_BYTE));
    let targetBitOffset = 0;
    for (let chunkIndex = 0; chunkIndex < this.chunks.length; chunkIndex += 1) {
      const length = this.lengths[chunkIndex];
      this.chunks[chunkIndex].copyInto(output, targetBitOffset, length);
      targetBitOffset += length;
    }
    return output;
  }
