...
export class DictionaryColumnStorage<Values extends readonly string[]> implements ColumnStorage<Values[number]> {
  ...
  constructor(private readonly values: Values, capacity: number, data?: DictionaryCodeArray, rowCount = data?.length ?? 0, private readonly chunkSize = DEFAULT_CHUNK_SIZE) {
    ...
    if (data !== undefined) {
      if (data.length !== capacity) throw new ColQLError("COLQL_INVALID_SERIALIZED_DATA", `Dictionary column data length ${data.length} does not match capacity ${capacity}.`);
      for (let index = 0; index < Math.min(rowCount, data.length); index += 1) this.appendCode(data[index]);
    }
  }
  ...
  encode(value: Values[number]): number {
    ...
    if (code === undefined) throw new ColQLError("COLQL_UNKNOWN_VALUE", `Invalid value for dictionary column "dictionary": expected one of ${JSON.stringify(this.values)}, received ${String(value)}.`);
    return code;
  }
  ...
  set(rowIndex: number, value: Values[number]): void {
    ...
    while (rowIndex > this.currentRowCount) this.appendCode(0);
    if (rowIndex === this.currentRowCount) return this.appendCode(code);
    const { chunkIndex, offset } = this.locate(rowIndex);
